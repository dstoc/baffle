                    ...
                    match hyper::upgrade::on(&mut req).await {
                        Ok(upgraded) => {
                            ...
                            while bytes_read < buffer.len() {
                                match upgraded.read(&mut buffer[bytes_read..]).await {
                                    Ok(0) => break,
                                ...
                                .await
                            {
                                if buffer[..2] == *b"\x16\x03" {
                                    ...
                                    // Do not turn a failed interception attempt into an
                                    // opaque tunnel. A caller can require HTTP-level policy
                                    // checks for this CONNECT destination.
                                    return;
                                }
                            }
