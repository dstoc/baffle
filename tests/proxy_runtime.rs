...
// Intercepted HTTP/1.1 and HTTP/2 request-policy fixtures run in the Rama
// runtime's unit suite, which provides a trusted local TLS origin for Rama's
// upstream handshake. This keeps the asserted boundary on denied HTTP requests,
// independent of when the runtime opens its authorized upstream TLS connection.
