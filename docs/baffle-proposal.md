...
- Each rule specifies permitted destination ports; if omitted, the rule uses HTTPS port 443. A CONNECT authority must include an explicit port, including `:443` for the default HTTPS port. Reject a rule that lists port 80. A non-default TLS port must be listed explicitly. A port number does not prove that an opaque tunnel carries TLS.
