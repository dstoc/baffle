...
the destination connection. Baffle will reject ordinary plaintext HTTP proxy
requests and plaintext `http://` destinations, including a request made after
a client follows an HTTPS-to-HTTP redirect. Baffle does not follow redirects
itself.
...
injection. It injects a credential only into a successfully intercepted
request that passes the host, port, TLS identity, and path checks. It never
injects into a plaintext request or an opaque tunnel.
...
failed TLS interception instead of opening an opaque fallback. Such a fallback
would bypass path checks and the credential-injection boundary. If a client
sends its own credentials through a tunnel, Baffle cannot inspect or constrain
those credentials.
...
and rejects opaque fallback when interception is required. The current
behavior is documented in the [configuration reference](docs/configuration.md)
